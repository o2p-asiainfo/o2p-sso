...
<html>
<head>
	<title><s:text name="eaap.op2.sso.customer.modifyCustomer"/></title>
...
<!--	<script type="text/javascript" src="${contextPath}/resource/plugins/artDialog/iframeTools.js" ></script> -->
	...
	<script type="text/javascript">
	   ...
	   function formSubmit(){
			...
			var sysCustomerId = ${SysCustomer.sysCustomerId};
			var sysCustomerNameReg = /^.{1,50}$/;
			if(sysCustomerNameReg.test(sysCustomerName)){
				$.ajax({
					type: "post",
					url: "${contextPath}/sysCustomer/isUpdateCustomerExist.shtml",
					data: {
					    sysCustomerId : sysCustomerId,
					    sysCustomerName : sysCustomerName
					},
					dataType: "json",
	   ...
	   <div class="contentWarp">
		  ...
		  <div class="module-path">
	  		  <div class="module-path-content">
		    	  ...
		    	  <img src="${contextPath}/resource/${contextStyleTheme}/images/module-path.png" /><s:text name="eaap.op2.sso.customer.modifyCustomer"/>
<!--		    	  <s:property value="%{getText('eaap.op2.conf.server.manager.choice')}" />-->
	      ...
	      <div>
	      	   <form name="myForm" id="myForm" method="post" action="${contextPath}/sysCustomer/updateSysCustomer.shtml?sysCustomerId=${SysCustomer.sysCustomerId}">
	 		   	<table class="mgr-table">
					<tr>
	  					<td class="item"><s:text name="eaap.op2.sso.customer.customerName"/>:</td>
	   					<td class="item-content">
	   						<div class="ui-widget">
	  							<ui:inputText skin="${contextStyleTheme}" name="sysCustomerName" id="sysCustomerName" value="${SysCustomer.sysCustomerName}"></ui:inputText>
	  						</div>  	
	   					</td>
	   					<td class="item"><s:text name="eaap.op2.sso.customer.area"/>:</td>
			   			<td class="item-content">
			   				<div class="ui-widget">
			  				    <ui:select skin="${contextStyleTheme}" name="sysAreaName" id="sysAreaName" value="${SysCustomer.sysAreaId}" headerValue=" " list="areaList"  emptyOption="true" listKey="sysAreaId" listValue="sysAreaName" onchange="getChangeData(this.value);"></ui:select>
			  				</div>  	
			   			</td>
			   			<td class="item"><s:text name="eaap.op2.sso.customer.status"/>:</td>
			   			<td class="item-content">
			   				<div class="ui-widget">
			  				    <ui:select skin="${contextStyleTheme}" name="state" id="state" value="${SysCustomer.sysStatusId}" list="stateList"  emptyOption="true" listKey="stateId" listValue="stateName" onchange="getChangeData(this.value);"></ui:select>
			  				</div>  	
			   			</td>
	   				</tr>
	    	    </table>
