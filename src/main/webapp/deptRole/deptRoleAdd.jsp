...
<html>
<head>
	<title><s:text name="eaap.op2.sso.deptRole.addNewDeptRole"/></title>
...
<!--	<script type="text/javascript" src="${contextPath}/resource/plugins/artDialog/iframeTools.js" ></script> -->
	...
	<script type="text/javascript">
		...
		function formSubmit(){
			var sysDeptId = $("#deptName").val();
			//var sysRoleIds = $("#roleName").multiselect("MyValues");
			var sysRoleIds = $("#roleName").val();
			if(sysDeptId != "" && sysRoleIds !=""){
				$("#myForm").submit();
			}else{
				alert("<s:text name='eaap.op2.sso.deptRole.deptRoleNotNull'/>");
			}
		}
		
		function getChangeData(value){
			var paramsStr = "sysDeptId:" + value;
			var url = "${contextPath}/rainbowui/getMulSelectBoxData.shtml?t=&method=eaap-op2-sso-sysDeptRoleAction.queryNotSelectedRole&params="+paramsStr+"&listKey=SYS_ROLE_ID&listValue=SYS_ROLE_NAME";
	   ...
	   <div class="contentWarp">
		  ...
		  <div class="module-path">
	  		  <div class="module-path-content">
		    	  ...
		    	  <img src="${contextPath}/resource/${contextStyleTheme}/images/module-path.png" /><s:text name="eaap.op2.sso.deptRole.addNewDeptRole"/>
<!--		    	  <s:property value="%{getText('eaap.op2.conf.server.manager.choice')}" />-->
	          </div>
	      </div>
		  <div class="accordion-header" >
		      <div class="accordion-header-text">
		      	  <span><span class="accordion-header-icon" > &nbsp;&nbsp;&nbsp;&nbsp;</span><s:text name="eaap.op2.sso.deptRole.deptRoleInfo"/></span>  <!--<s:property value="%{getText('eaap.op2.conf.techimpl.queryArea')}" />-->
		      </div>
	      </div>
	      <div>
	      	   <form name="myForm" id="myForm" method="post" action="${contextPath}/sysDeptRole/addSysDeptRole.shtml">
	 		   	<table class="mgr-table">
					<tr>
	  					<td class="item"><font color="red">*</font><s:text name="eaap.op2.sso.deptRole.dept"/>:</td>
			   			<td class="item-content">
			   				<div class="ui-widget">
			  				    <ui:select skin="${contextStyleTheme}" name="deptName" id="deptName" list="deptList"  emptyOption="true" headerValue=" "	 listKey="deptId" listValue="deptName" onchange="getChangeData(this.value);"></ui:select>
			  				</div>  	
			   			</td>
	   					
	   					<td class="item"><font color="red">*</font><s:text name="eaap.op2.sso.deptRole.role"/>:</td>
			   			<td class="item-content">
			   				<div class="ui-widget">
			   			        <ui:multiSelectBox id="roleName" name="roleName" params="" width="225" layerWidth="217" loadMode="true" list=""  
			  				     method="eaap-op2-sso-sysDeptRoleAction.queryNotSelectedRole" listKey="SYS_ROLE_ID" listValue="SYS_ROLE_NAME">
			  				    </ui:multiSelectBox>
			  				</div>  	
			   			</td>
	   				</tr>
	    	    ...
	    	    <table class="mgr-table">
    		    	<tr>
    		       		<td colspan="6" class="item-content" align="center">
	    				    ...
	    				    <ui:button skin="${contextStyleTheme}"  text="%{getText('eaap.op2.sso.common.cancel')}"  onclick="location='${contextPath}/sysDeptRole/showSysDeptRole.shtml'"  id="submitId"></ui:button>        
    					</td>
    			 	</tr>
    		 	</table>
	           </form>
