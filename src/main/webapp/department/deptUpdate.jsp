...
<html>
<head>
	<title><s:text name="eaap.op2.sso.department.modifyDept"/></title>
	...
	<script type="text/javascript">
	   ...
	   function formSubmit(){
			...
			if(deptNameReg.test(deptName)){
				$.ajax({
					type: "post",
					url: "${contextPath}/sysDept/isUpdateDeptExist.shtml",
					data: {
					    deptId : deptId,
					    deptName : deptName
					},
	   ...
	   <div class="contentWarp">
		  ...
		  <div class="module-path">
	  		  <div class="module-path-content">
		    	 ...
		    	 <img src="${contextPath}/resource/${contextStyleTheme}/images/module-path.png" /><s:text name="eaap.op2.sso.department.modifyDept"/>
<!--		    	  <s:property value="%{getText('eaap.op2.conf.server.manager.choice')}" />-->
	      ...
	      <div>
	      	   <form name="myForm" id="myForm" method="post" action="${contextPath}/sysDept/updateSysDept.shtml?deptId=${sysDept.deptId}">
	 		   	<table class="mgr-table">
					<tr>
	  					<td class="item"><font color="red">*</font><s:text name="eaap.op2.sso.department.deptName"/>:</td>
	   					<td class="item-content">
	   						<div class="ui-widget">
	  							<ui:inputText skin="${contextStyleTheme}" name="deptName" id="deptName" value="${sysDept.deptName}"></ui:inputText>
	  						</div>  	
	   					</td>
	   					<td class="item"><s:text name="eaap.op2.sso.department.customer"/>:</td>
			   			<td class="item-content">
			   				<div class="ui-widget">
			  				    <ui:select skin="${contextStyleTheme}" name="sysCustomerName" id="sysCustomerName" value="${sysDept.sysCustomerId}" headerValue=" " list="customerList"  emptyOption="true" listKey="sysCustomerId" listValue="sysCustomerName" onchange="getChangeData(this.value);"></ui:select>
			  				</div>  	
			   			</td>
	   				</tr>
	   				<tr>
			   			...
			   			<td class="item-content">
			   				<div class="ui-widget">
			  				    <ui:select skin="${contextStyleTheme}" name="parentDeptName" id="parentDeptName"  value="${sysDept.parentDeptId}" headerValue=" " list="parentDeptList"  emptyOption="true"  listKey="parentDeptId" listValue="parentDeptName" onchange="getChangeData(this.value);"></ui:select>
			  				</div>  	
			   			</td>
			  			<td class="item"><font color="red">*</font><s:text name="eaap.op2.sso.department.status"/>:</td>
			   			<td class="item-content">
			   				<div class="ui-widget">
			  				    <ui:select skin="${contextStyleTheme}" name="state" id="state" value="${sysDept.sysStatusId}"  list="stateList"  emptyOption="true" listKey="stateId" listValue="stateName" onchange="getChangeData(this.value);"></ui:select>
			  				</div>  	
			   			</td>
	   				</tr>
	    	    </table>
