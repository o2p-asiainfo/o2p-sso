...
<html>
<head>
	<title><s:text name="eaap.op2.sso.roleFunction.addNewRoleFunction"/></title>
...
<!--	<script type="text/javascript" src="${contextPath}/resource/plugins/artDialog/iframeTools.js" ></script> -->
	...
	<script type="text/javascript">
		...
		function formSubmit(){
			var roleId = $("#sysRoleName").val();
			var functionIds = $("#functionName").multiselect("MyValues");
			if(roleId != "" && functionIds !=""){
				$("#myForm").submit();
			}else{
				alert("<s:text name='eaap.op2.sso.roleFunction.roleFunctionNotNull'/>");
			}
		}
		
		function getChangeData(value){
			var paramsStr = "roleId:" + value;
			var url = "${contextPath}/rainbowui/getMulSelectBoxData.shtml?t=&method=eaap-op2-sso-sysRoleFunctionAction.queryNotSelectedFunction&params="+paramsStr+"&listKey=FUNCTION_ID&listValue=FUNCTION_NAME";
			$("#functionName").multiselect({url: url}).multiselect("refresh");
	   ...
	   <div class="contentWarp">
		  ...
		  <div class="module-path">
	  		  <div class="module-path-content">
		    	  ...
		    	  <img src="${contextPath}/resource/${contextStyleTheme}/images/module-path.png" /><s:text name="eaap.op2.sso.roleFunction.addNewRoleFunction"/>
<!--		    	  <s:property value="%{getText('eaap.op2.conf.server.manager.choice')}" />-->
	          </div>
	      </div>
		  <div class="accordion-header" >
		      <div class="accordion-header-text">
		      	  <span><span class="accordion-header-icon" > &nbsp;&nbsp;&nbsp;&nbsp;</span><s:text name="eaap.op2.sso.roleFunction.roleFunctionInfo"/></span>  <!--<s:property value="%{getText('eaap.op2.conf.techimpl.queryArea')}" />-->
		      </div>
	      </div>
	      <div>
	      	   <form name="myForm" id="myForm" method="post" action="${contextPath}/sysRoleFunction/addSysRoleFunction.shtml">
	 		   	<table class="mgr-table">
					<tr>
	  					<td class="item"><font color="red">*</font><s:text name="eaap.op2.sso.roleFunction.role"/>:</td>
	   					...
	   					<td class="item"><font color="red">*</font><s:text name="eaap.op2.sso.roleFunction.function"/>:</td>
			   			<td class="item-content">
			   				<div class="ui-widget">
			   			        <ui:multiSelectBox id="functionName" name="functionName" params="" width="225" layerWidth="217" loadMode="true" list=""  
			  				     method="eaap-op2-sso-sysRoleFunctionAction.queryNotSelectedFunction" listKey="FUNCTION_ID" listValue="FUNCTION_NAME">
			  				    </ui:multiSelectBox>
			  				</div>  	
			   			</td>
	   				</tr>
	    	    ...
	    	    <table class="mgr-table">
    		    	<tr>
    		       		<td colspan="6" class="item-content" align="center">
	    				    ...
	    				    <ui:button skin="${contextStyleTheme}"  text="%{getText('eaap.op2.sso.common.cancel')}"  onclick="location='${contextPath}/sysRoleFunction/showSysRoleFunction.shtml'"  id="submitId"></ui:button>        
    					</td>
    			 	</tr>
    		 	</table>
	           </form>
