...
(function($){
	...
	view = {
		...
		cancelPreSelectedNode: function (setting, node) {
			...
			for (var i=0, j=list.length-1; j>=i; j--) {
				if (!node || node === list[j]) {
					...
					$("#" + list[j].tId ).removeClass(consts.node.CURSELECTED+"Li"); //by cl
					if (node) {
		...
		makeDOMNodeMainBefore: function(html, setting, node) {
			...
			if (node.isParent) {
				html.push(" node_",node.open ? consts.folder.OPEN : consts.folder.CLOSE);
			} else {
				html.push(" node_", consts.folder.DOCU); 
			}
			html.push("' tabindex='0' hidefocus='true' treenode>");
		},
		...
		selectNode: function(setting, node, addFlag) {
			...
			//alert("#" + node.tId + "#" + node.tId + consts.id.A);
			$("#" + node.tId ).addClass(consts.node.CURSELECTED+"Li"); //by cl
